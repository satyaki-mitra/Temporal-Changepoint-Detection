...
class ChangePointDetectionConfig(BaseModel):
    ...
    """
    # EXECUTION CONTROL
    execution_mode               : Literal['single', 'compare']            = Field(default     = 'compare',
    ...
    hazard_lambda                : float                                   = Field(default     = 100.0,
                                                                                   ge          = 10.0,
                                                                                   le          = 1000.0,
                                                                                   description = "Expected segment length (hazard parameter)",
                                                                                  )

    likelihood_model             : Literal['gaussian', 'student_t']        = Field(default     = 'gaussian',
                                                                                   description = "BOCPD likelihood model",
                                                                                  )

    prior_mean                   : float                                   = Field(default     = 0.0,
                                                                                   description = "Prior mean for BOCPD",
                                                                                  )

    prior_variance               : float                                   = Field(default     = 1.0,
                                                                                   ge          = 1e-6,
                                                                                   description = "Prior variance for BOCPD",
                                                                                  )
    ...
    def validate_minimum_segment_size(cls, v):
        if v < 5:
            raise ValueError("minimum_segment_size must be ≥5 for statistical validity")
        return v

    @root_validator
    def validate_execution(cls, values):
        mode      = values.get('execution_mode')
        detectors = values.get('detectors')

        if ((mode == 'single') and (len(detectors) != 1)):
            raise ValueError("execution_mode='single' requires exactly one detector")

        if ((mode == 'compare') and (len(detectors) < 2)):
    ...
    def get_summary(self) -> dict:
        ...
        if 'bocpd' in self.detectors:
            summary['BOCPD'] = {'Hazard lambda'       : self.hazard_lambda,
                                'Likelihood model'    : self.likelihood_model,
                                'Prior mean'          : self.prior_mean,
                                'Prior variance'      : self.prior_variance,
