...
class ModelSelector:
    ...
    def select(self, raw_results: Dict[str, Dict]) -> Dict:
        ...
        if (len(models) < 2):
            best_model = list(models.keys())[0] if models else None
            
            return {'best_model'  : best_model,
                    'ranking'     : list(models.keys()),
                    'scores'      : {m: 1.0 for m in models.keys()},
                    'explanation' : "Only one model available—selection trivial",
                   }        
