...
from src.detection.penalty_tuning import tune_penalty_bic
from src.detection.statistical_tests import PELTStatisticalValidator
...
class PELTDetector:
    ...
    def __init__(self, config, cost_model = None):
        ...
        ----------
            ...
            cost_model       { str }              : Name of the cost function to be used in PELT
        """
        self.config     = config

        self.cost_model = cost_model or config.pelt_cost_models[0]
    ...
    def detect(self, aggregated_signal: np.ndarray) -> Dict:
        ...
        # Run PELT
        pelt_algorithm = rpt.Pelt(model    = self.cost_model,
                                  min_size = self.config.minimum_segment_size,
                                  jump     = self.config.jump,
                                 )
